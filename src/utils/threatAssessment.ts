...
export function calculateThreatLevel(
  ...
  const highSeverityRisks = districtRisks.filter(r => 
    r.severity === 'high' || r.severity === 'critical'
  ).length;
  const mediumSeverityRisks = districtRisks.filter(r => 
    r.severity === 'medium'
  ).length;
  
  const documentedRisks = Math.min(
    (highSeverityRisks * 15) + (mediumSeverityRisks * 5),
    40
  );
