...
export function identifyHotspots(
  ...
  const sortedByIndex = [...biodiversityData].sort(
    (a, b) => b.biodiversityIndex - a.biodiversityIndex
  );
  const sortedByRichness = [...biodiversityData].sort(
    (a, b) => b.speciesRichness - a.speciesRichness
  );

  const indexThreshold =
    sortedByIndex[Math.floor(biodiversityData.length * 0.2)]?.biodiversityIndex || 0;
  const richnessThreshold =
    sortedByRichness[Math.floor(biodiversityData.length * 0.2)]?.speciesRichness || 0;

  // Identify hotspots
  const hotspots = biodiversityData
    .filter(d => d.biodiversityIndex >= indexThreshold && d.speciesRichness >= richnessThreshold)
    .map(d => {
 ...
 */
export function isHotspot(districtData: BiodiversityData, allData: BiodiversityData[]): boolean {
 ...
 */
export function getHotspotRank(districtId: number, hotspots: BiodiversityHotspot[]): number | null {
