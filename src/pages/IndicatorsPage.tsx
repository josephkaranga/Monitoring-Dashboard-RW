...
const TIER_SAMPLES: Record<IndicatorTier, string[]> = {
  headline: [
    '% of land/water under biodiversity-inclusive spatial plans',
    'Area (ha) of degraded land under restoration',
    'Coverage of PAs and OECMs (% of land/sea)',
  ],
  component: [
    '% of spatial plans utilising KBA information',
    'Areas of degraded wetland ecosystems restored',
    'Area (ha) of Key Biodiversity Areas effectively conserved',
  ],
  complementary: [
    'Population density in biodiversity-sensitive areas',
    'Socioeconomic drivers of land-use change',
    'Governance quality index for protected areas',
  ],
  binary: [
    'National legislation for IAS adopted (Y/N)',
    'National biodiversity finance plan in place (Y/N)',
    'NBSAP submitted to CBD Secretariat (Y/N)',
  ],
};

const TIER_CFG: Record<
  IndicatorTier,
  { label: string; color: string; bg: string; border: string; icon: string; faIcon: string }
> = {
  headline: {
    label: 'Headline',
    color: '#166534',
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <h3 style={{ fontSize: '1rem', fontWeight: 700, flex: 1, paddingRight: 16 }}>
            #{String(i.id).padStart(3, '0')} — {i.name}
          </h3>
          <button
            onClick={onClose}
            style={{
        ...
        <div style={{ padding: 24 }}>
          <div style={{ display: 'flex', gap: 8, marginBottom: 16 }}>
            ...
            >
              <i className={`fa-solid ${tier.faIcon}`} style={{ fontSize: '0.58rem' }} />{' '}
              ...
              >
                ...
                >
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    ...
                    <span style={{ fontSize: '1.1rem', fontWeight: 700, color: mpc.color }}>
                      {i.progress}%
                    </span>
                  </div>
                </div>
                ...
                >
                  <span>
                    Current: <strong style={{ color: '#fff' }}>{i.current_value}</strong>
                  </span>
                  <span>
                    Target:{' '}
                    <strong style={{ color: '#fff' }}>{i.final_target || i.target_2030}</strong>
                  </span>
                </div>
              </div>
            );
          ...
          >
            {[
              ...
              ['Target 2030', i.target_2030],
              ['Baseline (2020)', i.baseline],
              ['Midterm 2027', i.midterm],
            ].map(([label, value]) => (
              <div
                key={label}
                style={{ background: 'var(--surface-2)', borderRadius: 10, padding: 14 }}
              >
                <div
                  style={{
                    fontSize: '0.65rem',
          ...
          >
            <strong>Data Source:</strong> {i.data_source}
            <br />
            <strong>Frequency:</strong> {i.periodicity} &nbsp;·&nbsp; <strong>KM-GBF:</strong>{' '}
            {i.km_gbf}
          </div>
          ...
          <div style={{ marginTop: 14, display: 'flex', gap: 8 }}>
            <button
              onClick={() => {
                onClose();
                onViewTarget(i.nbsap_target_id);
              }}
              style={{
                display: 'flex',
                alignItems: 'center',
...
export default function IndicatorsPage() {
  ...
  const { data: rawIndicators, loading } = useIndicators() as {
    data: Indicator[] | null;
    loading: boolean;
    error: string | null;
    refetch: () => void;
  ...
  const grouped = useMemo(() => {
    ...
    filtered.forEach(i => {
      const k = `Target ${i.nbsap_target_id}`;
      if (!map[k]) map[k] = [];
      map[k].push(i);
    });
    return Object.entries(map).sort(
      (a, b) => parseInt(a[0].replace('Target ', '')) - parseInt(b[0].replace('Target ', ''))
    );
    ...
    <div>
      ...
      >
        ...
        >
          ...
          <div
            style={{ fontSize: '2rem', fontWeight: 700, fontFamily: "'Playfair Display', serif" }}
          >
            {indicators.length}
          </div>
        ...
        {(Object.entries(TIER_CFG) as [IndicatorTier, (typeof TIER_CFG)[IndicatorTier]][]).map(
          ([key, cfg]) => (
            <div
              key={key}
              onClick={() => setTierFilter(t => (t === key ? 'all' : key))}
              style={{
                background: tierFilter === key ? cfg.bg : 'var(--surface)',
                border: `1px solid ${tierFilter === key ? cfg.border : 'var(--border)'}`,
                borderRadius: 'var(--radius)',
                padding: 16,
                cursor: 'pointer',
                transition: '0.2s',
            ...
            >
              <div
                style={{ fontSize: '0.72rem', color: cfg.color, fontWeight: 600, marginBottom: 4 }}
              >
                {cfg.label}
              </div>
              <div
              ...
              <div style={{ fontSize: '0.65rem', color: cfg.color, opacity: 0.7, marginTop: 2 }}>
                indicators
              </div>
              <i
                className={`fa-solid ${cfg.faIcon}`}
                style={{
                  position: 'absolute',
                  right: 16,
                  top: '50%',
      ...
      <div
        style={{
          ...
          padding: 18,
          marginBottom: 16,
          borderLeft: '4px solid var(--sky-dim)',
          ...
          {(Object.entries(TIER_CFG) as [IndicatorTier, (typeof TIER_CFG)[IndicatorTier]][]).map(
            ([key, cfg]) => {
              ...
              const TIER_DESCRIPTIONS: Record<IndicatorTier, string> = {
                headline:
                  'High-level metrics tracking overall progress toward each GBF target. One primary headline indicator per national target. Required for CBD national reporting.',
                component:
                  'Detailed metrics providing nuanced information on specific aspects of target implementation. 2–4 per target, supporting evidence-based adaptive management.',
                complementary:
                  'Contextual metrics that support interpretation of headline and component indicators, including socioeconomic variables influencing biodiversity outcomes.',
                binary:
                  'Yes/no assessments of whether specific policy or legal conditions have been met (e.g., whether national legislation for IAS is adopted).',
              };
              return (
                <div
                  key={key}
      ...
      >
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12 }}>
          {(Object.entries(GOAL_CFG) as [GBFGoalKey, (typeof GOAL_CFG)[GBFGoalKey]][]).map(
            ([gKey, gcfg]) => {
              ...
              return (
                ...
                >
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                    {(['headline', 'component', 'complementary', 'binary'] as IndicatorTier[]).map(
                      tier => (
                        <div
                          key={tier}
                          style={{
      ...
      >
        <div style={{ position: 'relative', flex: 1, maxWidth: 320 }}>
          ...
          <input
            type="text"
            placeholder={`Search all ${indicators.length} indicators…`}
            value={search}
          ...
          {[
            ...
            <button
              ...
              style={{
                ...
                cursor: 'pointer',
                fontFamily: "'DM Sans', sans-serif",
                background: tierFilter === val ? 'var(--navy)' : 'var(--surface)',
                color: tierFilter === val ? '#fff' : 'var(--text-2)',
                transition: '0.2s',
              }}
            >
              {lbl}
        ...
        <select
          ...
          style={{
            ...
            fontSize: '0.78rem',
            fontFamily: "'DM Sans', sans-serif",
            background: goalFilter ? 'var(--navy)' : 'var(--surface)',
            color: goalFilter ? '#fff' : 'var(--text-2)',
            cursor: 'pointer',
            outline: 'none',
          }}
        >
        ...
        <button
          ...
          style={{
            ...
            cursor: 'pointer',
            fontFamily: "'DM Sans', sans-serif",
            background: groupByTarget ? 'var(--navy)' : 'var(--surface)',
            color: groupByTarget ? '#fff' : 'var(--text-2)',
            display: 'flex',
            alignItems: 'center',
            gap: 5,
          }}
        ...
        >
          ...
          >
            ...
            {filtered.length === indicators.length
              ? `All ${indicators.length} NBSAP Indicators`
              : `${filtered.length} indicators matching filters`}
          </div>
        </div>
        {loading ? (
          ...
          <div style={{ padding: 16 }}>
            {grouped.map(([targetKey, inds]) => (
              <div key={targetKey} style={{ marginBottom: 16 }}>
                <div
                  id={`group-target-${targetKey.replace('Target ', '')}`}
                  style={{
                    fontSize: '0.8rem',
                    fontWeight: 700,
                    marginBottom: 8,
                ...
                >
                  <i className="fa-solid fa-bullseye" style={{ color: 'var(--sky-dim)' }} />{' '}
                  {targetKey}
                  <span
                    style={{
                      fontSize: '0.65rem',
                  ...
                  <button
                    onClick={() => navigate(`/targets?expand=${targetKey.replace('Target ', '')}`)}
                    style={{
                      marginLeft: 'auto',
                      fontSize: '0.68rem',
                      padding: '2px 10px',
                ...
                {inds.map(ind => (
                  <IndicatorRow key={ind.id} indicator={ind} onClick={() => handleOpen(ind)} />
                ))}
              </div>
            ))}
          </div>
                  ...
                  return (
                    ...
                    >
                      ...
                      <td style={{ padding: '11px 14px' }}>
                        ...
                        >
                          <i
                            className={`fa-solid ${tier.faIcon}`}
                      ...
                      >
                        <span
                          onClick={e => {
                            e.stopPropagation();
                            jumpToTarget(ind.nbsap_target_id);
                          }}
                          style={{
                            cursor: 'pointer',
                            color: 'var(--sky-dim)',
...
function IndicatorRow({ indicator: ind, onClick }: { indicator: Indicator; onClick: () => void }) {
  ...
  return (
    ...
    >
      ...
      >
        <i className={`fa-solid ${tier.faIcon}`} style={{ marginRight: 3, fontSize: '0.55rem' }} />
        {ind.tier}
      </span>
