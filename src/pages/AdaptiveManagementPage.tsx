    ...
    <div>
      ...
      <div style={{ ...card, padding: 20, marginBottom: 24 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, flexWrap: 'wrap', padding: '8px 0' }}>
          {[
            ...
            <React.Fragment key={step.num}>
              <div style={{ background: step.bg, color: '#fff', padding: '14px 16px', borderRadius: 8, textAlign: 'center', minWidth: 120 }}>
        ...
        <div style={{ marginTop: 16, display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 10 }}>
          {[
            { bg: '#fee2e2', border: '#f43f5e', color: '#991b1b', label: 'Critical', desc: 'Indicator <40% of milestone — emergency ministerial review + budget reallocation within 30 days' },
            { bg: '#fff7ed', border: '#f59e0b', color: '#9a3412', label: 'Warning', desc: 'Indicator 40–60% of milestone — technical review + corrective action plan within 60 days' },
            ...
            <div key={t.label} style={{ background: t.bg, borderRadius: 8, padding: '12px 14px', borderLeft: `3px solid ${t.border}` }}>
              <div style={{ fontSize: '0.72rem', fontWeight: 700, color: t.color, marginBottom: 4 }}>{t.label}</div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 14, marginBottom: 24 }}>
        {[
          { color: '#f43f5e', title: 'Implementation Gap Detection', desc: `Early warning system for indicators falling behind targets. Currently ${(stats?.atRiskIndicators ?? 0) + (stats?.behindIndicators ?? 0)} indicators flagged for corrective action.` },
          { color: '#10b981', title: 'Restoration Performance', desc: `Monitoring forest and wetland restoration against milestones. Currently at ${forestPct}% of 2,500 ha 2030 target.` },
          { color: '#f59e0b', title: 'Financing Shortfall Monitoring', desc: `Tracking biodiversity budget allocations vs disbursements. ${financePct} mobilized.` },
          { color: '#6366f1', title: 'Institutional Compliance', desc: 'Evaluating ministry and district reporting compliance. 92% EIA, 88% Protected Areas, 75% ABS, 68% Species Laws.' },
          { color: '#0ea5e9', title: 'Evidence-Based Policy Adjustment', desc: 'Enabling dynamic response to biodiversity risks. 28 decisions made from M&E data out of 50 target decisions by 2030.' },
          { color: '#ec4899', title: 'Gender-Responsive Monitoring', desc: 'Tracking women, youth & local community inclusion in biodiversity M&E. Currently 32% women and 18% local community participation.' },
        ].map(c => (
        ...
        <div style={{ position: 'relative', paddingLeft: 24 }}>
          ...
          {[
            ...
            <div key={item.title} style={{ position: 'relative', paddingLeft: 20, marginBottom: 18 }}>
              ...
              <div style={{ fontSize: '0.72rem', color: item.color, fontWeight: 700, marginBottom: 2 }}>{item.label}</div>
