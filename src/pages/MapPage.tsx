...
function getTooltipContent(
  ...
  switch (layer) {
    ...
    case 'protected-areas':
      return districtData
        ? `${districtData.protectedAreaCoverage?.toFixed(1)}% coverage`
        : 'No data';
    case 'wetlands':
...
export function MapPage() {
  ...
  const {
    data: nbsapProgressData,
    loading: nbsapLoading,
    refetch: refetchNBSAP,
  } = useNBSAPProgress();
  ...
  const zoomIn = useCallback(() => {
    setMapView(v => {
      ...
      return {
        x: v.x + (v.width - newW) / 2,
        y: v.y + (v.height - newH) / 2,
        width: newW,
        height: newH,
      };
    });
  ...
  const handleGBIFHover = useCallback(
    (occurrence: GBIFOccurrence | null) => {
      debouncedHover(() => {
  ...
  const handleProtectedAreaHover = useCallback(
  ...
  const handleRiverHover = useCallback(
  ...
  const handleLakeHover = useCallback(
    ...
    const fetchGeoData = async () => {
      try {
        ...
        if (!geoJson.features || geoJson.features.length === 0) {
          ...
          console.log(
            `  Longitude range: ${RWANDA_BOUNDS.minLon}° to ${RWANDA_BOUNDS.maxLon}° (West to East)`
          );
          console.log(
            `  Latitude range: ${RWANDA_BOUNDS.minLat}° to ${RWANDA_BOUNDS.maxLat}° (South to North)`
          );
          console.log(
            `  SVG viewBox: x=${RWANDA_VIEWBOX.x}, y=${RWANDA_VIEWBOX.y}, width=${RWANDA_VIEWBOX.width}, height=${RWANDA_VIEWBOX.height}`
          );
          console.log(
            `  Coordinate transformation: Latitude values are negated for correct north-up orientation`
          );
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
              {isMobile ? (
                <>
                  ...
                  >
                    <i
                      className={`fa-solid fa-${controlsExpanded ? 'chevron-up' : 'chevron-down'}`}
                    />
                    Controls
                  </button>
                  {controlsExpanded && (
                    ...
                    >
                      ...
                      >
                        <span>Overlays</span>
                        <i
                          className={`fa-solid fa-${overlaysExpanded ? 'chevron-up' : 'chevron-down'}`}
                        />
                      </button>
                      {overlaysExpanded && (
                        <OverlayToggles
          ...
          >
            ...
            {error && (
              ...
              >
                <i
                  ...
                  style={{ fontSize: '2rem', color: '#f43f5e', marginBottom: 12 }}
                />
            ...
            {exportError && (
              ...
              >
                ...
                <div style={{ fontSize: '0.8rem', color: '#991b1b', flex: 1 }}>{exportError}</div>
                <button
        ...
        <div style={{ ...card, padding: isMobile ? 12 : 18 }}>
          ...
          {searchQuery.length >= 2 &&
            (() => {
              ...
              const matchedAreas = (protectedAreas?.features || []).filter(a =>
                a.properties.name.toLowerCase().includes(q)
              );
              const matchedRivers = (rivers?.features || []).filter(r =>
                r.properties.name.toLowerCase().includes(q)
              );
              const matchedLakes = (lakes?.features || []).filter(l =>
                l.properties.name.toLowerCase().includes(q)
              );
              const hasOverlayResults =
                matchedAreas.length + matchedRivers.length + matchedLakes.length > 0;
                  ...
                  {matchedAreas.map((a, i) => {
                    ...
                    return (
                      <div
                        key={`pa-${i}`}
                        onClick={() => {
                          zoomToCoords(cLon, cLat, Math.max(size * 0.8, 0.1));
                          setSearchQuery('');
                        }}
                        style={{
                          display: 'flex',
                  ...
                  {matchedRivers.map((r, i) => {
                    const coords = (
                      r.geometry.type === 'LineString'
                        ...
                        : (r.geometry.coordinates as number[][][])[0]
                    ) as number[][];
                  ...
                  {matchedLakes.map((l, i) => {
                    ...
                    return (
                      <div
                        key={`lk-${i}`}
                        onClick={() => {
                          zoomToCoords(cLon, cLat, Math.max(size * 0.8, 0.08));
                          setSearchQuery('');
                        }}
                        style={{
                          display: 'flex',
                      ...
                      >
                        ...
                        >
                          {l.properties.area_km2 ? `${l.properties.area_km2} km²` : ''}
                        </span>
                      </div>
                    );
                  })}
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 16 }}>
                  ...
                  background:
                    {
                      ...
                      T06: '#922B21',
                      T07: '#1A5276',
                    }[selectedMapReport.tool_id] || '#6b7280',
                  display: 'flex',
                  alignItems: 'center',
                  justifyContent: 'center',
                  color: '#fff',
              ...
              <div>
                ...
                >
                  ...
                  {selectedMapReport.submitted_by_profile?.full_name &&
                    ` · ${selectedMapReport.submitted_by_profile.full_name}`}
