...
import type { UserSettings, NotificationPreferences } from '../types/index';
  ...
  >
    ...
    <span
      style={{
        ...
        transition: '0.2s',
        border: checked ? '1px solid #059669' : '1px solid var(--border)',
      }}
    >
      <span
        style={{
...
export default function SettingsPage() {
  ...
  const [settings, setSettings] = useState<Partial<UserSettings>>({
    ...
    species_fuzzing: false,
    mask_species_names: false,
    restrict_raw_export: true,
    log_exports: true,
    language: 'en',
  });
  ...
  const { data: rawAuditEntries } = useAuditLog({
    actionType: auditFilter === 'all' ? undefined : auditFilter,
  });
  const auditEntries = rawAuditEntries ?? [];

  useEffect(() => {
    if (!user?.id) return;
    fetchNotifPreferences(user.id).then(p => {
      if (p) setNotifPrefs(p);
    });
  }, [user?.id]);
  ...
  const tabs = [
    ...
    'Notifications',
    'Security',
    ...(permissions?.canViewAuditLog ? ['Audit Log'] : []),
  ];
          ...
          {tabs.map(tab => (
            <button
              ...
              style={{
                ...
                border: 'none',
                borderBottom:
                  tab === activeTab ? '2px solid var(--sky-dim)' : '2px solid transparent',
                marginBottom: -1,
                background: 'none',
                fontSize: '0.8rem',
                fontWeight: 600,
                cursor: 'pointer',
                color: tab === activeTab ? 'var(--sky-dim)' : 'var(--text-3)',
                fontFamily: "'DM Sans', sans-serif",
                transition: '0.2s',
              }}
            >
        ...
        <div style={{ padding: '20px 24px' }}>
          ...
          {activeTab === 'General' && (
            <div>
              ...
              <SettingRow
                label="Show Live Toolkit Stats"
                sub="Display real-time T01–T07 submission counts on dashboard"
              >
                <Toggle
                  checked={!!settings.show_live_stats}
                  onChange={v => setSettings(s => ({ ...s, show_live_stats: v }))}
                />
              </SettingRow>
              <SettingRow label="Animate Progress Bars" sub="Animate bars on tab load">
                <Toggle
                  checked={!!settings.animate_bars}
                  onChange={v => setSettings(s => ({ ...s, animate_bars: v }))}
                />
              </SettingRow>
              <SettingRow label="Compact Sidebar" sub="Hide label text, show icons only">
                <Toggle
                  checked={!!settings.compact_sidebar}
                  onChange={v => setSettings(s => ({ ...s, compact_sidebar: v }))}
                />
              </SettingRow>
              ...
              <SettingRow label="Auto-refresh Dashboard" sub="Refresh metrics every 60 seconds">
                <Toggle
                  ...
                  onChange={v => setSettings(s => ({ ...s, auto_refresh: v }))}
                />
              </SettingRow>
              <SettingRow
                label="Show Baseline Data on Charts"
                sub="Include 2020 baseline in trend charts"
              >
                <Toggle
                  checked={!!settings.show_baseline}
                  onChange={v => setSettings(s => ({ ...s, show_baseline: v }))}
                />
              </SettingRow>
              <SettingRow
                label="Require Verification Before Dashboard Update"
                sub="New toolkit submissions stay pending until approved by REMA reviewer"
              >
                <Toggle
                  checked={!!settings.require_verification}
                  onChange={v => setSettings(s => ({ ...s, require_verification: v }))}
                />
              </SettingRow>
              ...
              <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
                ...
                <button
                  onClick={() =>
                    toast('Restore: import a JSON backup file via the Reporting Toolkit page', {
                      icon: 'ℹ️',
                    })
                  }
                  style={{
            ...
            <div>
              ...
              <div
                style={{
                  ...
                  padding: '12px 14px',
                  marginBottom: 14,
                  borderLeft: '3px solid var(--sky-dim)',
                }}
              >
                <div
                  style={{
                ...
                <div style={{ fontSize: '0.72rem', color: '#0369a1', marginTop: 4, opacity: 0.8 }}>
                  Contact REMA admin to change your role
                </div>
              </div>
            ...
            <div>
              ...
              <div style={{ marginBottom: 14 }}>
                <label style={labelStyle}>
                  ...
                  >
                    (for deadline &amp; compliance alerts)
                  </span>
                </label>
                <input
                  type="email"
                  placeholder="your.name@rema.gov.rw"
              ...
              {[
                {
                  key: 'sub_overdue',
                  label: 'Overdue District Reports',
                  desc: 'Alert when districts have not submitted within the reporting window',
                },
                {
                  key: 'sub_compliance',
                  label: 'Compliance Below Threshold',
                  desc: 'Alert when an indicator drops below the warning level',
                },
                {
                  key: 'sub_deadlines',
                  label: 'Upcoming Report Deadlines',
                  desc: 'Alert N days before a reporting deadline',
                },
                {
                  key: 'sub_pending',
                  label: 'New Submissions Pending Verification',
                  desc: 'Notify when toolkit submissions are awaiting REMA review',
                },
                {
                  key: 'sub_finance',
                  label: 'Finance Disbursement Gap',
                  desc: 'Alert when disbursement falls below allocation by more than 30%',
                },
                {
                  key: 'sub_risk',
                  label: 'High-Risk Trigger Activated',
                  desc: 'Alert when a High-level risk in the Risk Register is triggered',
                },
              ].map(({ key, label, desc }) => (
                <SettingRow key={key} label={label} sub={desc}>
                  <Toggle
                    checked={!!notifPrefs[key as keyof NotificationPreferences]}
          ...
          {activeTab === 'Security' && (
            <div>
              ...
              <SettingRow
                label="Enable Species Location Fuzzing"
                sub="Blur exact GPS coordinates of sensitive species observations to ~5 km radius before display or export. Applies to T03 Protected Area data."
              >
                <Toggle
                  checked={!!settings.species_fuzzing}
                  onChange={v => setSettings(s => ({ ...s, species_fuzzing: v }))}
                />
              </SettingRow>
              <SettingRow
                label="Mask Sensitive Species Names in Public View"
                sub="Replace threatened species names with generic taxonomy in public-access exports"
              >
                <Toggle
                  checked={!!settings.mask_species_names}
                  onChange={v => setSettings(s => ({ ...s, mask_species_names: v }))}
                />
              </SettingRow>

              <SectionHead>Export Controls</SectionHead>
              <SettingRow
                label="Restrict Raw Data Export by Role"
                sub="Public users can only export aggregated summaries. Raw records require Institutional or Decision-Maker role."
              >
                <Toggle
                  checked={!!settings.restrict_raw_export}
                  onChange={v => setSettings(s => ({ ...s, restrict_raw_export: v }))}
                />
              </SettingRow>
              <SettingRow
                label="Log All Data Exports"
                sub="Record every export action in the Audit Log with user role and timestamp"
              >
                <Toggle
              ...
              >
                <div style={{ display: 'grid', gridTemplateColumns: 'auto 1fr', gap: '4px 14px' }}>
                  ...
                  <span>
                    Headline indicators, national summaries, aggregated maps. No data entry. No raw
                    export.
                  </span>
                  <span style={{ fontWeight: 700, color: '#1e40af' }}>🏛 Institutional</span>
                  <span>
                    Own ministry/district data entry and submission. Can view own submissions. No
                    approval rights.
                  </span>
                  <span style={{ fontWeight: 700, color: '#6b21a8' }}>📊 Decision-Maker</span>
                  <span>
                    Full dashboard, verification queue, all exports, AI narratives, risk register,
                    audit log.
                  </span>
                </div>
              </div>
              ...
              >
                <div>
                  ...
                  <div style={{ fontSize: '0.72rem', color: 'var(--text-3)', marginTop: 2 }}>
                    Tracks data access, exports, submissions, approvals and role changes. Stored in
                    the database.
                  </div>
                </div>
                    ...
                    return (
                      ...
                      >
                        ...
                        <div style={{ flex: 1 }}>
                          ...
                          {e.detail && (
                            <div
                              style={{ color: 'var(--text-2)', fontSize: '0.72rem', marginTop: 1 }}
                            >
                              {e.detail}
                            </div>
                          )}
