...
export type AIExtractionStatus = 'pending' | 'processing' | 'complete' | 'failed';
export type AIProposalType =
  ...
  | 'budget_update'
  | 'geographic_update';
export type AIProposalStatus = 'pending' | 'approved' | 'rejected' | 'modified';
...
export interface AIMilestoneFind {
  description: string;
  milestone_keyword: string;
  confidence: number;
}
...
export interface AIBudgetInfo {
  amount: number;
  currency: string;
  type: 'allocated' | 'utilized' | 'disbursed' | 'gap';
  source: string;
}

export interface AIChallengeExtract {
  type: string;
  description: string;
  delay_mentioned?: string;
}

export interface AIRiskExtract {
  description: string;
  severity: 'high' | 'medium' | 'low';
}

export interface AIExtractionResult {
  targets_mentioned: number[];
  progress_updates: AIProgressUpdate[];
  milestone_completions: AIMilestoneFind[];
  budget_info: AIBudgetInfo[];
  locations: string[];
  activities: string[];
  challenges: AIChallengeExtract[];
  risks: AIRiskExtract[];
...
export interface AIExtraction {
  ...
  raw_text_analyzed?: string;
  document_text?: string;
  extraction_result: AIExtractionResult;
  error_message?: string;
  processing_time_ms?: number;
  model_used?: string;
  proposals?: AIExtractionProposal[];
  created_at: string;
  updated_at: string;
}
