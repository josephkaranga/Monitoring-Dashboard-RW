...
export function TestRBISHooks() {
  ...
  const { metrics, recentOccurrences, loading: metricsLoading, error: metricsError } = useRBISMetrics(false);
  ...
  return (
    <div style={{ padding: '20px', fontFamily: 'monospace' }}>
      ...
      <div style={{ marginBottom: '20px', padding: '10px', border: '1px solid #ccc' }}>
        ...
        <button onClick={connection.status === 'connected' ? disconnect : connect} disabled={connLoading}>
          {connLoading ? 'Loading...' : connection.status === 'connected' ? 'Disconnect' : 'Connect'}
        </button>
      </div>
      ...
      <div style={{ marginBottom: '20px', padding: '10px', border: '1px solid #ccc' }}>
        ...
        {metrics && (
          <>
            <p>Total Occurrences: <strong>{metrics.totalOccurrences.toLocaleString()}</strong></p>
            <p>Last 24 Hours: <strong>{metrics.last24Hours.toLocaleString()}</strong></p>
            <p>Last 7 Days: <strong>{metrics.last7Days.toLocaleString()}</strong></p>
            <p>Active Data Streams: <strong>{metrics.activeDataStreams}</strong></p>
            <p>Recent Occurrences: <strong>{recentOccurrences.length}</strong></p>
          </>
        )}
      </div>
      ...
      <div style={{ marginBottom: '20px', padding: '10px', border: '1px solid #ccc' }}>
        ...
        {goals && (
          <>
            <p>GBF Goals: <strong>{goals.length}</strong></p>
            {goals.map(goal => {
      ...
      <div style={{ marginBottom: '20px', padding: '10px', border: '1px solid #ccc' }}>
        ...
        {dataStreams && (
          <>
            <p>Total Data Streams: <strong>{dataStreams.length}</strong></p>
      ...
      <div style={{ marginBottom: '20px', padding: '10px', border: '1px solid #ccc' }}>
        ...
        {summary && (
          <>
            <p>Total Targets: <strong>{summary.totalTargets}</strong></p>
            <p>Total Indicators: <strong>{summary.totalIndicators}</strong></p>
            <p>Linked Indicators: <strong>{summary.linkedIndicators}</strong> ({summary.linkagePercentage}%)</p>
            <p>On Track: <strong style={{ color: 'green' }}>{summary.onTrackIndicators}</strong></p>
            <p>At Risk: <strong style={{ color: 'orange' }}>{summary.atRiskIndicators}</strong></p>
            <p>Off Track: <strong style={{ color: 'red' }}>{summary.offTrackIndicators}</strong></p>
            <p>Active Data Streams: <strong>{summary.activeDataStreams}</strong></p>
            <p>Total Occurrences: <strong>{summary.totalOccurrences.toLocaleString()}</strong></p>
          </>
        )}
      </div>
      ...
      <div style={{ padding: '10px', backgroundColor: '#d4edda', border: '1px solid #c3e6cb' }}>
        ...
        <ul>
          <li>✅ Database tables created (rbis_linkages, rbis_data_streams, rbis_connection_log)</li>
