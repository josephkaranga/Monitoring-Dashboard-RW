      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 20, flexWrap: 'wrap' }}>
        {['pending', 'approved', 'rejected', 'stale', 'cancelled'].map(status => (
          <button
            ...
            style={{
              padding: '8px 16px',
              border: filter === status ? '2px solid #0ea5e9' : '1px solid #e2e8f0',
              borderRadius: 8,
              background: filter === status ? '#e0f2fe' : '#fff',
              cursor: 'pointer',
              fontSize: '0.85rem',
              fontWeight: 600,
              color: filter === status ? '#0369a1' : '#64748b',
              textTransform: 'capitalize',
            }}
          >
            {status}
          ...
          {requests.map(request => {
            ...
            return (
              <div
                ...
                style={{
                  background: '#fff',
                  border: isStale ? '2px solid #f472b6' : '1px solid #e2e8f0',
                  borderRadius: 12,
                  padding: 20,
                }}
              >
                ...
                {request.status === 'approved' && request.reviewer_name && (
                  ...
                  >
                    <strong>Approved by:</strong> {request.reviewer_name} on{' '}
                ...
                {request.status === 'rejected' && request.reviewer_name && (
                  ...
                  >
                    <strong>Rejected by:</strong> {request.reviewer_name} on{' '}
                    {new Date(request.reviewed_at!).toLocaleDateString()}
                    <div style={{ marginTop: 4 }}>
          ...
          >
            ...
            <div style={{ marginBottom: 16, padding: 12, background: '#f8fafc', borderRadius: 8 }}>
              <div style={{ fontSize: '0.85rem', color: '#64748b', marginBottom: 4 }}>
                <strong>{selectedRequest.requester_name}</strong> wants to change from{' '}
                <strong>{USER_ROLE_LABELS[selectedRequest.from_role]}</strong> to{' '}
                <strong>{USER_ROLE_LABELS[selectedRequest.to_role]}</strong>
              </div>
            </div>
