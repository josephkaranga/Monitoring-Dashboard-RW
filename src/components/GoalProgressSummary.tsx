  ...
  const goals = ['A', 'B', 'C', 'D'].map(g => {
    ...
    const avg =
      goalTargets.length > 0
        ? Math.round(goalTargets.reduce((s, t) => s + t.progress, 0) / goalTargets.length)
        : 0;
    return {
      goal: g,
      ...meta,
    ...
    >
      <div style={{ padding: '18px 22px', borderBottom: '1px solid #f1f5f9' }}>
        <div style={{ fontSize: '0.95rem', fontWeight: 700, color: '#0f172a' }}>
          Progress by GBF Goal
        </div>
        <div style={{ fontSize: '0.7rem', color: '#94a3b8', marginTop: 2 }}>
          Click a goal to see individual targets
        </div>
      </div>
          ...
          return (
            <div key={g.goal} style={{ marginBottom: isOpen ? 4 : 12 }}>
              ...
              <div
                ...
                onMouseEnter={e => {
                  if (!isOpen) e.currentTarget.style.borderColor = g.color + '60';
                }}
                onMouseLeave={e => {
                  if (!isOpen) e.currentTarget.style.borderColor = '#f1f5f9';
                }}
              >
                {/* Goal letter badge */}
                <div
