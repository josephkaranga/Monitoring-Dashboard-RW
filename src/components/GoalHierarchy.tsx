  ...
  return (
    <div style={{
      ...
      <div>
        ...
        {ind.periodicity && (
          <div style={{ fontSize: '0.58rem', color: '#94a3b8', marginTop: 2 }}>{ind.periodicity}</div>
        )}
      </div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 6, justifyContent: 'center' }}>
        ...
        <div>
          <div style={{ fontSize: '0.78rem', fontWeight: 800, color: pc.color }}>{ind.progress}%</div>
        </div>
      </div>
    </div>
  );
          ...
          <div key={g.key}>
            ...
            >
              <div style={{
                ...
                fontSize: '1.1rem', fontWeight: 800, fontFamily: "'Playfair Display', serif",
                flexShrink: 0,
              }}>
                {g.key}
              </div>
              ...
              <div style={{ flex: 1 }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 4 }}>
                  <div style={{ flex: 1, maxWidth: 200, height: 6, background: '#e2e8f0', borderRadius: 3, overflow: 'hidden' }}>
                    <div style={{ height: '100%', width: `${g.avg}%`, background: g.color, borderRadius: 3, transition: 'width 0.8s ease' }} />
                  </div>
                  <span style={{ fontSize: '0.72rem', fontWeight: 700, color: gpc.color }}>{g.avg}%</span>
                  <span style={{ fontSize: '0.62rem', color: '#94a3b8' }}>· {g.targets.length} targets</span>
                </div>
              </div>

              <i className={`fa-solid fa-chevron-${isGoalOpen ? 'up' : 'down'}`}
                style={{ color: '#94a3b8', fontSize: '0.7rem' }} />
            </div>

            {/* ── Targets under goal ── */}
            {isGoalOpen && g.targets.map(t => {
              ...
              return (
                <div key={t.id}>
                  ...
                  <div
                    ...
                    onMouseEnter={e => { if (!isTargetOpen) e.currentTarget.style.background = '#fafbfc'; }}
                    onMouseLeave={e => { if (!isTargetOpen) e.currentTarget.style.background = '#fff'; }}
                  >
                    <div style={{
                      width: 6, height: 32, borderRadius: 3,
                      background: tpc.color, flexShrink: 0,
                    }} />
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 4 }}>
                        <div style={{ flex: 1, maxWidth: 180, height: 5, background: '#f1f5f9', borderRadius: 3, overflow: 'hidden' }}>
                        ...
                        <span style={{
                          ...
                          background: tpc.bg, color: tpc.color,
                        }}>
                          {tpc.label}
                        </span>
                        {indicators.length > 0 && (
                    ...
                    <i className={`fa-solid fa-chevron-${isTargetOpen ? 'up' : 'down'}`}
                  ...
                  {isTargetOpen && indicators.length > 0 && (
                    <div style={{ background: '#fafbfc', borderBottom: '1px solid #f1f5f9' }}>
                      ...
                      <div style={{
                        display: 'grid', gridTemplateColumns: '1fr 100px 100px 80px',
                        gap: 12, padding: '6px 16px 6px 68px',
                        borderBottom: '1px solid #f1f5f9',
                      }}>
                        <div style={{ fontSize: '0.58rem', fontWeight: 600, color: '#94a3b8', textTransform: 'uppercase', letterSpacing: '0.04em' }}>Indicator</div>
                        <div style={{ fontSize: '0.58rem', fontWeight: 600, color: '#94a3b8', textTransform: 'uppercase', letterSpacing: '0.04em', textAlign: 'center' }}>Target</div>
                        <div style={{ fontSize: '0.58rem', fontWeight: 600, color: '#94a3b8', textTransform: 'uppercase', letterSpacing: '0.04em', textAlign: 'center' }}>Current</div>
                        <div style={{ fontSize: '0.58rem', fontWeight: 600, color: '#94a3b8', textTransform: 'uppercase', letterSpacing: '0.04em', textAlign: 'center' }}>Progress</div>
                      </div>
                  ...
                  {isTargetOpen && indicators.length === 0 && (
                    <div style={{ padding: '12px 22px 12px 68px', background: '#fafbfc', borderBottom: '1px solid #f1f5f9' }}>
                      <div style={{ fontSize: '0.72rem', color: '#94a3b8', fontStyle: 'italic' }}>No indicators linked to this target</div>
                    </div>
                  )}
                </div>
              );
