...
interface MetricConfig {
  ...
  getDistrictBreakdown: (
    districts: District[]
  ) => { name: string; value: number; detail: string }[];
  districtUnit: string;
  inverse?: boolean;
}
...
const configs: Record<Exclude<MetricKey, null>, MetricConfig> = {
  ...
  finance: {
    ...
    unit: 'M RWF',
    getCurrent: (m, s) =>
      m.financeUtilizedMillionRwf || (s?.financeAllocated ? s.financeAllocated / 1e6 : 0),
    getTarget: (m, _t) => m.financeAllocatedMillionRwf || m.financeUtilizedMillionRwf || 0,
    targetLabel: (t, u) =>
      t > 0 ? `Allocated budget: ${t.toFixed(1)} ${u}` : 'Biodiversity finance allocation',
    getDistrictBreakdown: districts =>
      districts
        .map(d => ({
          name: d.name,
    ...
    getDistrictBreakdown: districts =>
      districts
        .map(d => ({
          ...
          detail:
            d.status === 'submitted'
              ? 'Active — submitting reports'
              : d.status === 'pending'
                ? 'Pending — reports in review'
                : 'Not yet reporting',
        }))
        .sort((a, b) => b.value - a.value),
    districtUnit: 'status',
      ...
      >
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '20px 24px' }}>
          ...
          >
            ...
            <div>
              <div
                style={{ height: 10, background: '#e2e8f0', borderRadius: 5, overflow: 'hidden' }}
              >
                <div
                  style={{
                    height: '100%',
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 4 }}>
                ...
                <span style={{ fontSize: '0.6rem', color: 'var(--text-3)' }}>
                  {isInverse
                    ? `${current.toLocaleString()} reported`
            ...
            >
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 8 }}>
                {[
                  {
                    label: 'Allocated',
                    value: `${m.financeAllocatedMillionRwf.toFixed(1)}M`,
                    color: '#6366f1',
                  },
                  {
                    label: 'Disbursed',
                    value: `${m.financeDisbursedMillionRwf.toFixed(1)}M`,
                    color: '#0284c7',
                  },
                  {
                    label: 'Utilized',
                    value: `${m.financeUtilizedMillionRwf.toFixed(1)}M`,
                    color: '#059669',
                  },
                ].map(item => (
                  <div key={item.label} style={{ textAlign: 'center', padding: '8px 4px' }}>
            ...
            {districtRows.length === 0 ? (
              ...
              >
                <i
                  ...
                  style={{ fontSize: '1.5rem', display: 'block', marginBottom: 8, opacity: 0.4 }}
                />
                  ...
                  return (
                    ...
                    >
                      ...
                      <div style={{ flex: 1 }}>
                        ...
                        >
                          <span
                            style={{ fontSize: '0.73rem', fontWeight: 600, color: 'var(--text-1)' }}
                          >
                            {d.name}
                          </span>
                          <span
                            style={{ fontSize: '0.7rem', fontWeight: 700, color: config.color }}
                          >
                        ...
                        <div style={{ fontSize: '0.56rem', color: 'var(--text-3)', marginTop: 2 }}>
                          ...
                          {totalVal > 0 &&
                            d.value > 0 &&
                            ` · ${Math.round((d.value / totalVal) * 100)}% of total`}
                        </div>
                      </div>
                    </div>
                  );
