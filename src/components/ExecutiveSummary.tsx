...
function GaugeRing({ value, size, stroke, color, bgColor }: { value: number; size: number; stroke: number; color: string; bgColor: string }) {
...
function StatusDonut({ onTrack, atRisk, behind, size = 140 }: { onTrack: number; atRisk: number; behind: number; size?: number }) {
      ...
      {segments.map((seg, i) => {
        ...
        return (
          <circle key={i} cx={size / 2} cy={size / 2} r={r} fill="none"
            ...
            strokeDashoffset={0}
            style={{ transform: `rotate(${rotation}deg)`, transformOrigin: 'center', transition: 'all 0.8s ease' }}
          />
        );
      })}
    </svg>
    ...
    <div style={{ marginBottom: 28 }}>
      <div style={{
        ...
        <div style={{ marginBottom: 24, textAlign: 'center' }}>
          <div style={{ fontSize: '1.15rem', fontWeight: 800, color: '#0f172a', letterSpacing: '-0.01em' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 24, alignItems: 'center' }}>
          ...
          <div style={{ textAlign: 'center' }}>
            <div style={{ position: 'relative', display: 'inline-block' }}>
              <GaugeRing value={avgProgress} size={150} stroke={14} color={pc.color} bgColor="#f1f5f9" />
              <div style={{
                position: 'absolute', inset: 0,
                display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center',
              }}>
                <div style={{ fontSize: '2.4rem', fontWeight: 800, color: pc.color, lineHeight: 1, fontFamily: "'Playfair Display', serif" }}>
                  {avgProgress}%
                </div>
          ...
          <div style={{ textAlign: 'center' }}>
            <div style={{ position: 'relative', display: 'inline-block' }}>
              ...
              <div style={{
                ...
                <div style={{ fontSize: '2.2rem', fontWeight: 800, color: '#0f172a', lineHeight: 1, fontFamily: "'Playfair Display', serif" }}>
                  {totalTgt}
                </div>
          ...
          <div style={{ textAlign: 'center' }}>
            <div style={{ position: 'relative', display: 'inline-block' }}>
              ...
              <div style={{
                ...
                <div style={{ fontSize: '0.65rem', color: '#64748b', marginTop: 2 }}>Indicators</div>
              </div>
            </div>
            <div style={{ fontSize: '0.78rem', fontWeight: 600, color: '#334155', marginTop: 10 }}>
            ...
            return (
              ...
              >
                ...
                <div>
                  ...
                  <div style={{ fontSize: '0.62rem', color: '#64748b' }}>{tCount} targets · {iCount} indicators</div>
                </div>
      ...
      {expanded && (() => {
        ...
        return (
          <div style={{
            background: '#fff', border: '1px solid #e2e8f0', borderTop: '1px dashed #cbd5e1',
            borderRadius: '0 0 14px 14px', padding: '20px 32px', maxHeight: 380, overflowY: 'auto',
          }}>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 28 }}>
              <div>
                <div style={{ fontSize: '0.82rem', fontWeight: 700, color: '#0f172a', marginBottom: 10, paddingBottom: 6, borderBottom: '1px solid #f1f5f9' }}>
                  {meta.label} Targets <span style={{ fontWeight: 400, color: '#94a3b8' }}>({tgts.length})</span>
                </div>
                {tgts.length === 0 ? (
                  <div style={{ padding: 20, textAlign: 'center', color: '#94a3b8', fontSize: '0.75rem', background: '#f8fafc', borderRadius: 8 }}>None</div>
                ) : tgts.map(t => {
                  const tpc = progressColor(t.progress);
                  const gc = goalColors[t.goal] ?? { bg: '#f1f5f9', color: '#334155' };
                  return (
                    <div key={t.id} style={{ padding: '10px 0', borderBottom: '1px solid #f8fafc' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
                        <span style={{ fontSize: '0.58rem', fontWeight: 700, padding: '2px 6px', borderRadius: 4, background: gc.bg, color: gc.color }}>Goal {t.goal}</span>
                        <span style={{ fontSize: '0.78rem', fontWeight: 600, color: '#0f172a', flex: 1 }}>T{t.id}: {t.title}</span>
                        <span style={{ fontSize: '0.78rem', fontWeight: 700, color: tpc.color }}>{t.progress}%</span>
                      </div>
                      <div style={{ height: 5, background: '#f1f5f9', borderRadius: 3, overflow: 'hidden' }}>
                        <div style={{ height: '100%', width: `${t.progress}%`, background: tpc.color, borderRadius: 3 }} />
                      </div>
                    </div>
                  );
                })}
              ...
              <div>
                <div style={{ fontSize: '0.82rem', fontWeight: 700, color: '#0f172a', marginBottom: 10, paddingBottom: 6, borderBottom: '1px solid #f1f5f9' }}>
                  {meta.label} Indicators <span style={{ fontWeight: 400, color: '#94a3b8' }}>({inds.length})</span>
                </div>
                {inds.length === 0 ? (
                  <div style={{ padding: 20, textAlign: 'center', color: '#94a3b8', fontSize: '0.75rem', background: '#f8fafc', borderRadius: 8 }}>None</div>
                ) : inds.map(ind => {
                  ...
                  return (
                    <div key={ind.id} style={{ padding: '10px 0', borderBottom: '1px solid #f8fafc' }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', gap: 8, marginBottom: 6 }}>
                        <span style={{ fontSize: '0.78rem', fontWeight: 600, color: '#0f172a', flex: 1 }}>{ind.name}</span>
                        <span style={{ fontSize: '0.78rem', fontWeight: 700, color: ipc.color, flexShrink: 0 }}>{ind.progress}%</span>
                      </div>
                      <div style={{ height: 5, background: '#f1f5f9', borderRadius: 3, overflow: 'hidden' }}>
