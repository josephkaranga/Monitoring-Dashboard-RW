...
export function useGBIFOccurrences({
  ...
  const fetchOccurrences = useCallback(
    async (isRetry = false, isManual = false) => {
      ...
      try {
        ...
        console.log(
          `[GBIF] ${completionTimestamp} - Refresh completed successfully: ${transformedOccurrences.length} occurrences fetched (total available: ${data.count || transformedOccurrences.length})`
        );
      } catch (err) {
        ...
        if (retryCountRef.current < MAX_RETRIES) {
          ...
          console.log(
            `[GBIF] ${errorTimestamp} - Retrying in ${delay}ms (attempt ${retryCountRef.current}/${MAX_RETRIES})`
          );
  ...
  useEffect(() => {
    ...
    console.log(
      `[GBIF] ${new Date().toISOString()} - Auto-refresh enabled (every ${intervalMinutes} minutes)`
    );

    refreshTimerRef.current = setInterval(() => {
      const timestamp = new Date().toISOString();
      console.log(
        `[GBIF] ${timestamp} - Auto-refresh triggered (${intervalMinutes}-minute interval)`
      );
...
export function useFilteredOccurrences(
  ...
  return kingdom ? occurrences.filter(occ => occ.kingdom === kingdom) : occurrences;
}
