...
import { getRoleChangeRequests, cancelRoleChangeRequest } from './roleChangeService';
import { useAuth } from './AuthContext';
import type { RoleChangeRequest } from './roleChangeService';
  ...
  if (requests.length === 0) {
    return (
      <div style={{ padding: 24, textAlign: 'center' }}>
        <i className="fa-solid fa-inbox" style={{ fontSize: 48, color: '#cbd5e1', marginBottom: 12 }} />
          ...
          <div key={request.id} style={{ position: 'relative', marginBottom: index < requests.length - 1 ? 24 : 0 }}>
            ...
            <div style={{ background: '#fff', border: '1px solid #e2e8f0', borderRadius: 12, padding: 16 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'start', marginBottom: 12 }}>
                ...
                {request.status === 'pending' && (
                  <button
                    ...
                    style={{ padding: '6px 12px', background: '#fee2e2', color: '#991b1b', border: 'none', borderRadius: 6, fontSize: '0.75rem', fontWeight: 600, cursor: cancelling === request.id ? 'not-allowed' : 'pointer', opacity: cancelling === request.id ? 0.7 : 1 }}
                  >
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr auto 1fr', gap: 12, alignItems: 'center', marginBottom: 12 }}>
                ...
                <div style={{ padding: 10, background: '#e0f2fe', borderRadius: 8, textAlign: 'center' }}>
              ...
              <div style={{ marginBottom: 12 }}>
                ...
                <div style={{ fontSize: '0.8rem', color: '#475569', lineHeight: 1.5 }}>{request.justification}</div>
              </div>

              {request.status === 'approved' && (
                <div style={{ padding: 10, background: '#d1fae5', borderRadius: 6, fontSize: '0.8rem', color: '#065f46' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 4 }}>
                    <i className="fa-solid fa-check-circle" />
                    <strong>Approved</strong>
                  </div>
                  <div>By {request.reviewer_name} on {new Date(request.reviewed_at!).toLocaleDateString()}</div>
                  {request.review_note && <div style={{ marginTop: 6, paddingTop: 6, borderTop: '1px solid #a7f3d0' }}><strong>Note:</strong> {request.review_note}</div>}
                </div>
              )}

              {request.status === 'rejected' && (
                <div style={{ padding: 10, background: '#fee2e2', borderRadius: 6, fontSize: '0.8rem', color: '#991b1b' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 4 }}>
                    <i className="fa-solid fa-times-circle" />
                    <strong>Rejected</strong>
                  </div>
                  <div>By {request.reviewer_name} on {new Date(request.reviewed_at!).toLocaleDateString()}</div>
                  <div style={{ marginTop: 6, paddingTop: 6, borderTop: '1px solid #fecaca' }}><strong>Reason:</strong> {request.rejection_reason}</div>
                </div>
              )}

              {request.status === 'cancelled' && (
                <div style={{ padding: 10, background: '#f3f4f6', borderRadius: 6, fontSize: '0.8rem', color: '#374151' }}>
              ...
              {request.status === 'stale' && (
                <div style={{ padding: 10, background: '#fce7f3', borderRadius: 6, fontSize: '0.8rem', color: '#831843' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
